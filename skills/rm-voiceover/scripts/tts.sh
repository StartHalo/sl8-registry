#!/usr/bin/env bash
# tts.sh — synthesize narration wav(s) via ai-gen TTS (Kokoro), keyless SL8 proxy.
#
#   bash tts.sh full      "<joined-narration>" <vo-dir> [voice] [model]   # ONE call -> narration.wav  (DEFAULT/batch path)
#   bash tts.sh "<text>"  <vo-dir> <beat-stem> [voice] [model]            # synth one beat (compat/fallback)
#   bash tts.sh concat    <vo-dir>                                        # concat beat wavs -> narration.wav (compat/fallback)
#
# e.g. bash tts.sh full "Ship faster. Then ship more." artifacts/teaser/assets/vo am_michael fal-ai/kokoro/american-english
#      bash tts.sh "Ship faster." artifacts/teaser/assets/vo beat-01 am_michael fal-ai/kokoro/american-english
...
# (ls <tmp>/*.wav), and move it to the target wav. ai-gen v2 JSON is { success, files[].local_path,
# hosted_urls[] } — we trust the on-disk wav and check success when jq/node is available.
#
# Exit 0 only when a non-empty target wav exists; non-zero on any TTS failure so the caller
# can take the documented SILENT fallback (never prompt the user).
set -uo pipefail

MODE_OR_TEXT="${1:?usage: tts.sh full \"<joined-narration>\" <vo-dir> [voice] [model]  |  tts.sh \"<text>\" <vo-dir> <beat-stem> [voice] [model]  |  tts.sh concat <vo-dir>}"

# run_tts: one ai-gen TTS call into a FRESH temp dir, capture the single wav (the `-o`-is-a-directory gotcha),
# check the v2 JSON success:false, and move the captured wav to $FINAL. Args: $1=label (for messages),
# $2=text, $3=final-wav-path, $4=voice, $5=model. Echoes $FINAL + returns 0 on success; non-zero on failure.
run_tts () {
  local LABEL="$1" TEXT="$2" FINAL="$3" VOICE="$4" MODEL="$5"
  if [ -z "${TEXT// /}" ]; then echo "!! empty narration text for $LABEL" >&2; return 2; fi
  ...
  if printf '%s' "$OUT_JSON" | grep -q '"success"'; then
    ...
    if [ "$SUCCESS" = "false" ]; then
      echo "!! ai-gen TTS reported success:false for $LABEL" >&2
  ...
  if [ -z "$WAV" ] || [ ! -s "$WAV" ]; then
    echo "!! no wav produced by ai-gen TTS for $LABEL (model=$MODEL voice=$VOICE)" >&2
    echo "$OUT_JSON" | tail -n 20 >&2
    rm -rf "$TMP"; return 1
  fi
...
# ---------- full mode (batch: ONE call -> narration.wav) ----------
if [ "$MODE_OR_TEXT" = "full" ]; then
  TEXT="${2:?missing joined narration text}"
  VO_DIR="${3:?missing vo-dir}"
  VOICE="${4:-am_michael}"
  MODEL="${5:-fal-ai/kokoro/american-english}"
  run_tts "narration" "$TEXT" "$VO_DIR/narration.wav" "$VOICE" "$MODEL"
  exit $?
fi
...
# ---------- synth mode (per beat; compat/fallback) ----------
TEXT="$MODE_OR_TEXT"
VO_DIR="${2:?missing vo-dir}"
...
run_tts "$STEM" "$TEXT" "$VO_DIR/$STEM.wav" "$VOICE" "$MODEL"
exit $?
