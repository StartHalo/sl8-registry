...
```jsonc
{
  ...
  "category": "funding|launch|partnership|hire|policy|research|other",
  "tone": "neutral",                                       // always neutral for news
  "recommended_style": "minimal-editorial",                // one of the 4 styles
...
- **`compressed`**: `true` if shortened from `source_span`; `false` must be a verbatim substring. Quotes and `primary_stat.value` are always verbatim.
- **Absent fields are `null`**, never empty-string-padded or guessed.

The render skill (`bot-012-news-video`) also accepts a **flat** shape where text fields are plain strings (e.g. `"headline": "..."`); the engine's `normalizeDoc` collapses either form. Writing the rich shape (with `source_span`) is preferred because it lets a grader prove nothing was invented.
