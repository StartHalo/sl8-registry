#!/usr/bin/env bash
#
# assemble.sh — normalize the per-scene Hailuo clips (and any still-segment fallbacks) to a
# uniform layout, concat them in scene order, ALWAYS add a subtle ambient/room-tone bed
# (Hailuo is SILENT — the bed is ALWAYS added here, never native), verify with ffprobe, and
# write episode.mp4 at the project root.
#
# This is the Hailuo first-last-frame assembler (BOT-029 phase 2). It mirrors BOT-028's Kling
# assembler: the room-tone bed is ALWAYS added (every Hailuo clip is silent, so there is no
# native audio to double up). The caller MUST state in summary.md that the audio is an added
# ambient bed, NOT native Hailuo audio.
#
# Donor lineage: BOT-028 kling-cinematic scripts/assemble.sh (normalize -> concat -> room-tone
# -> ffprobe) + BOT-027 seedance-cinematic scripts/per-shot-fallback.sh.
#
# Usage:
#   assemble.sh <project-dir>
#     <project-dir>  artifacts/<project-name> — clips are read from <project-dir>/work-scenes/
#                    scene-*.mp4 in lexicographic (= scene) order; episode.mp4 is written to
#                    <project-dir>/episode.mp4.
#
# Env knobs (all optional):
#   ASPECT       16:9 | 9:16 | 1:1   (default 16:9 — the normalize canvas)
#   AUDIO_DESC   the plan Audio: line (informational; recorded with the room tone)
#   ROOMTONE_DB  room-tone level in dB (default -38)
#
# Prints ONE JSON verdict line on stdout:
#   {"file":"...","route":"hailuo-first-last","scenes":N,"duration_s":S,"audio":"roomtone","verdict":"PASS|FLAG"}
...
[[ -d "$CLIPS_DIR" ]] || { err "clips directory not found: $CLIPS_DIR (run gen-keyframe-clips.sh first)"; exit 2; }

shopt -s nullglob
CLIPS=("$CLIPS_DIR"/scene-*.mp4)   # zero-padded scene-NN.mp4 → glob is sorted = scene order
shopt -u nullglob
[[ ${#CLIPS[@]} -ge 1 ]] || { err "no scene-*.mp4 clips in $CLIPS_DIR — nothing to assemble"; exit 2; }
...
# Hailuo clips are silent (as are the still-segment fallbacks); attach a silent stereo track
# so every segment has an identical A/V layout for the demuxer. A clip carrying native audio
# is still handled (for robustness), though in this pipeline none do.
i=0
...
# --- 2. concat in scene order (demuxer; re-encode on edge-case failure) -------------------
LIST="$TMP/concat.txt"
...
# --- 3. ambient/room-tone bed — ALWAYS added (Hailuo clips are silent) --------------------
# A quiet brown-noise room tone derived (in spirit) from the plan Audio: line. This is an
# ADDED ambient bed, NOT native Hailuo audio — disclose it in summary.md. The silent stereo
# track on each normalized clip is under-mixed with the bed.
[[ -n "$AUDIO_DESC" ]] && err "ambient bed derived from Audio: ${AUDIO_DESC:0:80}"
err "mixing brown-noise room tone at ${ROOMTONE_DB}dB under the episode (added ambient bed, NOT native Hailuo audio)"
...
# --- 4. ffprobe verify --------------------------------------------------------------------
# Expected total = sum of the per-clip durations (computed from the normalized clips so it
# tracks the actual snapped scene durations); tolerate ±2s for the whole episode.
SUM=$(python3 -c '
...
awk "BEGIN { d=$DUR; s=$SUM; exit !((d-s) <= 2.0 && (s-d) <= 2.0) }" && DUR_OK=true
...
err "ffprobe: duration=${DUR}s (summed scenes ${SUM}s, ±2s ok=$DUR_OK) video=${HAS_VIDEO:-none}(ok=$VID_OK) audio=${HAS_AUDIO:-none}(roomtone, ok=$AUD_OK) verdict=$VERDICT"

printf '{"file":"%s","route":"hailuo-first-last","scenes":%s,"duration_s":%.1f,"summed_s":%.1f,"audio":"roomtone","verdict":"%s"}\n' \
  "$EPISODE" "${#CLIPS[@]}" "$DUR" "$SUM" "$VERDICT"

[[ "$VERDICT" == PASS ]] \
  && err "episode verified: ${DUR}s, ${W}x${H}, ${#CLIPS[@]} scenes, ambient bed" \
