#!/usr/bin/env bash
# gen-image.sh — generate ONE image by walking the pinned character-bible model chain.
#
# Usage:
...
# has failed (the caller records the failure in generation-log.md / state.md and stops).
#
...
#     takes a resolution (1K/2K/4K, default 1K) — we forward --resolution when given.
...
command -v python3 >/dev/null 2>&1 || die "python3 not found on PATH (needed for JSON parsing — we parse files[0].local_path with python3 like the BOT-013 donor)"
...
die "all models in the bible chain failed for $STABLE_NAME (tried in order: ${MODELS[*]}). Record this in generation-log.md / state.md — do not improvise out-of-chain models."
