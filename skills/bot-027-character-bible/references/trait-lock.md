...
The craft reference for **phase A** of `bot-027-character-bible` (lock the spec). Everything a
character bible needs to hold one identity across many downstream **Seedance shots** is here —
baked **inline** from recipe family A (the runtime sandbox has **no KB access**). Load this
before composing tokens. (Adapted from BOT-016 `bot-016-character-design/references/trait-lock.md`;
the discipline is identical — only the downstream consumer changed from BOT-017/018 director bots
to this bot's own Seedance render phase.)

The single idea behind this whole phase: **identity comes from reference images + verbatim
...
byte-identical everywhere — in the bible images here, and in every `@Image`-anchored shot the
Seedance `reference-to-video` call renders in phase 3.
...
> CHARACTER_BLOCK, in the bible-image prompts (phase B), and in the multi-shot render prompt
> (phase 3). **Never paraphrase a locked token.**
...
half (never paraphrasing in a *downstream* prompt) is a discipline phase B inherits via the
frozen-block copy-paste contract — and which the phase-3 render skill inherits from the spec.
...
2. **5-7 trait locks** — the distinctive identity tokens, ordered **face → hair → eyes →
   outfit/props** (list face first, then hair, then eyes, then clothing). These become the
   Identity Tokens list.
3. **Pose / scene** — handled downstream per asset (the turnaround views, the hero portrait,
   and the Seedance shots); the spec stays pose-neutral.
4. **Style** — art style + render + lighting + camera look; becomes STYLE_STACK.
5. **Fixed seed** — one integer (default 7777), recorded once, reused across both bible images.
...
spec the pose lives downstream (a shot in phase 3), the identity is what we freeze here.
...
Seedance render keys its `@Image` reference on across cuts.
...
on more *reduces* it. This matters double here — the Seedance render also reads CHARACTER_BLOCK
across 4-6 shots, so a diluted lock drifts shot-to-shot, not just view-to-view.

**Positive framing only.** The downstream models (Gemini-family Nano Banana Pro / 2 for the
bible; Seedance for the render) respond badly to negation — write what the character *is*,
never "no beard, no glasses". Negatives, if ever needed, are a generation concern, not a token.
...
- "keep face and hairstyle identical … no text" → the downstream discipline phase B enforces
  on the bible images, and phase 3's render prompt inherits via "maintain the EXACT same
  identity in every shot".

**A2's rule:** change ONE variable at a time downstream (scene OR emotion OR outfit) — never
two at once. That is why identity is frozen in writing *before* any pixels: between the
turnaround views the only variable that changes is the camera angle; between Seedance shots
the only variable that changes is the action + camera, never the identity.

A1's turnaround-prompt shape (for context — phase B owns this, you just write the blocks it
...
what get pasted in front of that instruction, verbatim. (Full A1 template:
...
- **Default** (when the brief states no style):
  `"cinematic concept-art realism, dramatic lighting"`
  (the skill's documented `style-preference` default is `cinematic concept-art realism,
  dramatic lighting` — keep STYLE_STACK consistent with it).
...
- **Adapted from a stated preference** — keep the slot structure (style · render · lighting ·
  ...
  - brief says "Pixar-style 3D, bright and cheerful" →
    `"Pixar-style 3D animation, soft studio lighting, clean polished render, shallow depth of field"`
...
render measurably. **Lighting words earn the most quality-per-word** — both for the bible image
and for the Seedance render that will read this stack.

---

## 6. CHARACTER_BLOCK — composing the frozen identity line

CHARACTER_BLOCK is the Identity Tokens, **comma-joined in the fixed face → hair → eyes →
outfit/props order**, as one double-quoted line. Every token in it must be **byte-identical**
to its bullet in the Identity Tokens list — that byte-identity is the no-synonym lock the
validator checks and the thing that keeps the character the same across both bible views AND
across every Seedance shot.
...
This is the dark-elf bible the Step-0 PoC locked, in the exact spec shape phase A writes.
...
## Provenance
brief source: context.md · defaults applied: name coined "Vyre", seed 7777 · 2026-06-19 · models used: TBD (phase B records the sheet/hero models)
...
## Provenance
brief source: context.md · defaults applied: face/hair/outfit invented as neutral defaults (see note); name coined "Bean-Bot"; seed 7777 · 2026-06-19 · models used: TBD
...
- `SKILL.md` — the phase workflow that consumes this reference (phase A composes the spec from
  these rules; `scripts/validate-spec.sh` gates it).
- `references/nbp-dialect.md` — recipe A1 baked inline (the verbatim turnaround + hero prompt
  templates phase B pastes around these frozen blocks), the pinned image chain, per-model
  quirks, the `--resolution` fix, and the ai-gen v2.1.0 CLI mechanics.
