...
- **CHARACTER_BLOCK** (still prompt, block 2 — default; the locked one comes from `02-character/character-spec.md`): "An extremely minimal hand-drawn stick figure: a plain circle head, two small dot eyes, a simple curved smile, a small baseball cap worn slightly tilted, a small plain rounded torso drawn as one soft solid teardrop shape, and single-stroke arms and legs. No other facial features, no clothing besides the cap, no fingers — simple line hands. Proportions: head about one fifth of total height, limbs slightly longer than the torso. Every image of this character uses this exact same body construction."
