...
#
# character-spec.md is the FLEET INTERFACE: BOT-017/BOT-018 parse it by fixed section
# name, and phase 2 pastes its frozen blocks verbatim. This linter enforces the parts a
# machine can check (structure + the no-synonym byte-identity), zero LLM judgment.
#
...
#     face, hair, eyes (the fixed face->hair order); no empty token values
