#!/usr/bin/env bash
# validate-plan.sh — deterministic structural linter for continuous-plan.md
#
...
#
# Usage:  validate-plan.sh <path/to/continuous-plan.md>
# Exit:   0 = valid, 1 = lint errors (line-itemized on stdout), 2 = usage/deps
set -euo pipefail

usage() {
  echo "usage: validate-plan.sh <path/to/continuous-plan.md>" >&2
  exit 2
}
