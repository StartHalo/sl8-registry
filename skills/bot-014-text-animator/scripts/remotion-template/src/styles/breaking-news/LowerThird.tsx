...
//
// `hold` mode (used when the banner lives inside a <SceneSeries> sequence): the banner
// reveals in and then HOLDS — the cross-scene transition (wipe/slide) does the exit, so
// the banner must not self-wipe at the end of its (short) scene window.
// `variant="soundbite"` swaps the kicker slab to be the speaker credit for a quote scene.
...
export const LowerThird: React.FC<{
  ...
  hold?: boolean; // skip the self-wipe-out (the SceneSeries transition exits it)
}> = ({ headline, kicker, durationInFrames, hold = false }) => {
  ...
  const IN_END = 18;
  // In `hold` mode push the exit past the end of the window so the banner stays up and
  // the cross-scene transition handles the cut; otherwise reverse the wipe near the end.
  const OUT_START = hold ? durationInFrames + 1 : Math.max(IN_END + 6, durationInFrames - 18);
  const OUT_END = hold ? durationInFrames + 2 : durationInFrames;
