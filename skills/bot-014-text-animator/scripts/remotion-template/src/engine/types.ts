...
// shaped like ClipProps; the structure skill (script-builder) writes the (richer)
// RawMessageDoc. `normalizeDoc` collapses the rich, provenance-tracked MessageDoc into the
// flat RenderDoc the style components actually draw — so styles never touch provenance.
//
// BOT-014 generalizes BOT-012's news engine to ANY short message (announcement / update /
// quote / stat / headline). The doc shape is style-agnostic, so the *News*-prefixed names
// below are kept as back-compat aliases (RawNewsDoc / NewsVideoProps) — new code should
// prefer RawMessageDoc / MessageDoc / ClipProps.

export type StyleName =
  // The four original (BOT-012) looks, now all sequencer-driven.
  | "headline-highlight"
  | "breaking-news"
  | "kinetic-typography"
  | "minimal-editorial"
  // Five new looks (BOT-014, adapted from the Skiper references to time-based Remotion).
  | "box-reveal"
  | "giant-word"
  | "perspective-3d"
  | "pixel-reveal"
...
// ---- Rich NewsDoc as written by bot-014-script-builder (loose: accepts string or object) ----
...
// Generalized aliases — the doc is style-agnostic, so a "message" is just a news doc.
export type RawMessageDoc = RawNewsDoc;
export type MessageDoc = RenderDoc;
...
export type NewsVideoProps = ClipProps;
