...
> The non-negotiable for `rm-script`. Ported from BOT-015 `hf-script`, re-targeted to the Remotion phase
> chain. Mirrors BOT-014's fact-preserving MessageDoc. The craft rubric grades this directly (the
> `fidelity` dimension in `research/domain-analysis.md`). No rendering happens in this phase — but the
> numbers written here propagate all the way to the rendered pixels, so get them exactly right.
...
- **Numbers**: use the brief's figure exactly. Don't round (`$1.23M` ≠ `$1.2M` unless the brief
  ...
  exactly — `rm-dataviz` later animates a frame-driven `Counter`/`Bar` to these values with
  `tabular-nums`, so an error here propagates straight to the rendered pixels.
...
## 4. The restyle / resize / re-voice boundary (the only phase that changes facts)

`rm-script` (phase 2) is the **only** phase allowed to change facts. The phase chain re-enters at
different points for different changes (JTBD-4), and only this one touches the message:

| User asks for… | Re-run phase (skill) | Facts change? |
|---|---|---|
| "darker / bolder style", "different look" | build (`rm-build`) | **No** — re-author the React from the SAME script |
| "different voice", "re-voice it" | voiceover (`rm-voiceover`) | **No** |
| "make it 9:16", "resize", "re-render" | render (`rm-render`) — orientation change routes back to `rm-build` for a new `<Composition>` | **No** |
| "change the stat to 52%", "add a fact", "fix the name" | **script (`rm-script`, phase 2)** | **Yes** — re-script, re-run 3+ |

So: when the user asks to **restyle / re-voice / resize**, do NOT touch the script — those phases read
the *unchanged* facts. (A different aspect ratio is a separate `<Composition>`, not a fact change.) Only
when the user changes the **message itself** (adds, removes, or corrects a fact) do you re-run
`rm-script`. If a "restyle" request smuggles in a new fact ("make it bolder and say we doubled revenue"),
split it: re-script for the new fact (with provenance), then restyle. Never let a fact change ride in
silently on a cosmetic request.
