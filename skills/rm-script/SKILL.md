---
name: rm-script
description: "Write a faithful, beat-structured video script for a Remotion motion-graphics project — one beat per idea, each carrying a voiceover (VO) line and a short on-screen-text distillation, with every fact traced to the brief. No rendering — this is a text/authoring phase. Reads the project brief (context.md) and the base concept (01-concept.md); writes 02-script.md. On-screen text is the headline/keyword distillation, NOT the narration verbatim. Never invents facts, numbers, names, or quotes. Use during the SCRIPT phase (phase 2) of a narrated/data-viz video, after rm-concept and before rm-storyboard. This is the ONLY phase allowed to change facts — also use it to RE-SCRIPT when the user changes the message (JTBD-4)."
metadata:
  author: sl8
  version: 1.0.0
  references-skills: [rm-concept]
  inputs:
    - name: context
      type: markdown
      required: true
      description: "artifacts/[project]/context.md — the project brief (topic/script, audience, brand kit, source facts, requested duration/aspect ratio, voice/music defaults)."
    - name: concept
      type: markdown
      required: true
      description: "artifacts/[project]/01-concept.md — the 6-dimension base concept (subject, composition, style, palette, typography, mood) the script's tone and pacing align to."
  outputs:
    - name: script
      type: markdown
      path: artifacts/<project>/02-script.md
      description: "A beat-structured script — narrative arc, per-beat VO line + on-screen text + estimated seconds, plus a provenance table mapping every fact to its source. Faithful to the brief; no invented facts. Consumed by rm-storyboard (phase 3) and rm-voiceover (phase 4)."
---

# rm-script — beat-structured, fidelity-tracked video script

## Purpose
Turn the brief (`context.md`) and the base concept (`01-concept.md`) into a **beat-structured script**
at `artifacts/<project>/02-script.md`: a narrative arc broken into beats, where **each beat carries one
VO (voiceover) line and one short on-screen-text distillation**, paced to a duration. This is the
faithful "what the video says" layer that `rm-storyboard` (phase 3) maps onto Remotion blocks and
`rm-voiceover` (phase 4) reads its VO from. You write words, not React. **No rendering happens here.**
...
to change facts (see `references/fidelity-rule.md`); restyle/resize/re-voice (JTBD-4) read these facts
**frozen**.

`$SKILL` below = this skill's directory.

## When to use
- **Script** (phase 2): after `rm-concept` wrote `01-concept.md`, before `rm-storyboard`. Runs for the
  narrated brief path (JTBD-1) and the data-narrative path (JTBD-2).
...
- Do NOT use to design the visuals/Remotion blocks (that is `rm-storyboard`) or to generate the
  concept/palette (that is `rm-concept`). Do NOT use to silently fix facts during a restyle. A
  caption-only clip cut (JTBD-3) gets its words from ASR via `rm-voiceover`, not from this phase; a
  fully-custom generative video (JTBD-5) with no narration may **skip** script — but if it has VO, run
  this phase first.

## Inputs
- `artifacts/<project>/context.md` (required) — the brief: topic or pasted script, audience, the source
  facts (figures/names/quotes/feature claims), brand voice, requested duration + aspect ratio(s),
  voice/music defaults.
- `artifacts/<project>/01-concept.md` (required) — the base concept; align tone + pacing energy to its
  **mood** dimension (calm → slower beats, longer reads; upbeat/tech → tighter, punchier beats).
...
~2 s (a strong line or the single most striking number). For a data video (JTBD-2) use the
**hook → stat → stat → … → takeaway** arc, one quarter/figure per stat beat.
...
- **On-screen text** — the **distillation**: the headline or 1–4 keywords/the bare number for that beat,
  ...
  **p99 −47%**.) Mark the **focal token** the storyboard should emphasize — a stat focal token signals
  `rm-storyboard` to animate it as the engine's `Counter` primitive (a `stat` TypeKey), a verb/brand word
  signals a `RiseIn`/`headline`.
- **Estimated seconds** — the read-time, so the beat seconds sum to ~the target duration. Note the running
  total; if it overshoots, tighten VO or drop a beat (don't speed past legibility). Downstream these
  seconds become frames at the engine's **FPS = 30** (`engine/tokens.ts`), and the composition's timeline
  must end exactly at `durationInFrames` — so a clean total here keeps the render contract honest.
...
`references/fidelity-rule.md` for what counts as a fact vs framing, and the restyle/resize/re-voice rule.
...
= more, tighter beats). Then write `artifacts/<project>/02-script.md` using the structure in
`references/scriptwriting-beats.md` → "Output format": a title + the resolved assumptions (duration,
audience, narrated/silent), the **arc** named, a **per-beat table** (`# · sec · VO line · on-screen text
· focal token`), the stated **total**, and the **provenance table**.

### 6. Self-check (structural gate, no network)
Run the bundled structural self-check before handing off to `rm-storyboard`:

```bash
bash "$SKILL/scripts/check-script.sh" artifacts/<project>/02-script.md
```

It verifies the four required sections (Assumptions / Arc / Beats / Provenance), the beats-table contract
columns, at least one numbered beat row, and a stated total. Exit 0 = structurally valid; exit 2 = fix the
listed gaps and re-run. This is a shape check only — fidelity (no invented facts) is yours to enforce per
step 4.

## Outputs
- `artifacts/<project>/02-script.md` — the beat-structured script: assumptions block (resolved defaults),
  the named narrative arc, a per-beat table (beat #, estimated seconds, VO line, on-screen text, focal
  token), running/total duration, and a provenance table (fact → source in the brief). Faithful to the
  brief; no invented facts; on-screen text distilled (not the VO verbatim). This is the only artifact this
  skill writes — it does **not** write a storyboard, a Remotion composition, or any render.
...
Beat 3 (proof) VO "We cut p99 latency by forty-seven percent." on-screen **p99 −47%** (focal: 47% → a
`Counter`). Provenance: 47% ← brief. Seconds sum ≈ 15. No new numbers introduced.
...
table lists all four figures ← the CSV. The displayed numbers `rm-dataviz` animates in later phases must
equal these exactly — so write them exactly, no rounding the brief didn't ask for.
...
`02-script.md`, and note that phases 3+ (storyboard/voiceover/build) must re-run from the new facts.
...
- **User asks to "make it punchier" / restyle / resize / re-voice** → that does NOT change facts; do not
  re-script. Only re-script when the user changes the message itself (a fact). (`references/fidelity-rule.md`.)
- **check-script.sh exits 2** → a required section, the beats-table columns, or a beat row is missing;
  fix exactly what it prints and re-run before phase 3.
...
- [ ] Written to `artifacts/<project>/02-script.md`; `scripts/check-script.sh` exits 0.
