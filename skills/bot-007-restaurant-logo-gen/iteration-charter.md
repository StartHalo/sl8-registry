...
rubric:
  dimensions:
    ...
    - id: logo-vocabulary-internalization
      weight: 0.04
      source: llm-judge
      jtbd_source: JTBD-1
      judge_prompt: |
        Score 0-10 whether the bot's base concept and per-model prompts internalize the v1.2.0 logo design vocabulary. A 10 means: base concept declares its Wheeler type by name (one of: Wordmark / Lettermark / Pictorial mark / Abstract mark / Mascot / Combination mark / Emblem); the concept text uses ≥4 of {wordmark, lockup, mark, signature, letterform, counterform, negative space, stroke weight, optical alignment} as design-precise terms, not as vague substitutes; favicon-test and B&W-test self-checks are present in the base concept block. A 5 means: Wheeler type is named OR vocabulary is partially used but key terms are missing; only one of the two self-checks is present. A 0 means: no Wheeler type, no anatomy vocabulary, no self-checks — concept reads as generic "design a nice logo" prose.

    - id: legibility-test-coverage
      weight: 0.03
      source: llm-judge
      jtbd_source: JTBD-2
      judge_prompt: |
        Score 0-10 whether comparison.md § Per-Model Observations includes operationalized legibility-test verdicts per surviving model. A 10 means: every surviving model has named verdicts for all three Step 8b tests — Favicon test (32x32 readability), B&W test (monochrome readability), and Single-mark-clarity test — each with a one-sentence justification naming what reads / what disappears / where the mark competes with secondary elements. A 5 means: 1-2 of the 3 tests are present per model, OR all 3 are present but the justifications are bare ("passes" / "fails" with no naming). A 0 means: no legibility-test verdicts anywhere in comparison.md — the Step 8b output is absent.
