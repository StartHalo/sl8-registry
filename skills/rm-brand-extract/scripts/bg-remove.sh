...
#   e.g. bash bg-remove.sh artifacts/acme/assets/captures/brand-shot.png artifacts/acme/assets/cutouts/logo.png
#
# Runs:  ai-gen run fal-ai/bria/background/remove --image <logo-image>
# ai-gen v2 JSON: { "success": bool, "files": [{ "local_path": "..." }], "hosted_urls": [...] }
# (`ai-gen run` takes a POSITIONAL model id; the matte lands under ~/artifacts/remove-<ts>.png — we
# read files[0].local_path from the JSON, NOT a guessed path). Keyless via SL8_SESSION_TOKEN, ~1 credit.
# This is the same bg-remove the rm-assets skill ships; kept here so brand extraction runs standalone.
#
# On success:false, an unreachable proxy, or a missing input, EXIT NON-ZERO so the skill omits the
# logo line (the brand block still has accent/fontPack/label). NEVER prompts. No auth, no cloud render.
set -uo pipefail
...
# Parse the v2 JSON: require success!=false and a files[0].local_path.
